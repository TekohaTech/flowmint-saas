...
let AuthService = class AuthService {
    ...
    prisma;
    emailService;
    constructor(usuariosService, jwtService, prisma, emailService) {
    ...
    async validateUser(username, pass) {
        ...
        if (user && user.pass && (await bcrypt.compare(pass, user.pass))) {
            if (user.rol?.nombre !== 'SUPERADMIN') {
                if (!user.comercio) {
                    throw new common_1.UnauthorizedException('El usuario no tiene un comercio asociado');
                }
                if (!user.comercio.activo) {
                    throw new common_1.UnauthorizedException('Tu comercio se encuentra inactivo. Contacta al administrador.');
                }
            }
    ...
    async completarRegistro(dto) {
        ...
        if (user.comercio_id) {
            throw new common_1.BadRequestException('El usuario ya tiene un comercio asociado');
        }
    ...
    async register(dto, ip) {
        ...
        if (!controlIP.puedeRegistrar && process.env.NODE_ENV === 'production') {
            ...
            throw new common_1.HttpException(`Ya realizaste un intento de registro. Intenta nuevamente en ${minutosRestantes} minutos`, common_1.HttpStatus.TOO_MANY_REQUESTS);
        }
    ...
    async verificarEmail(token) {
        ...
        if (!usuario) {
            throw new common_1.BadRequestException('Token de verificación inválido o expirado.');
        }
        await this.prisma.usuario.update({
    ...
    async verificarControlIP(ip, correo) {
